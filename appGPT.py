...
class AttentionUNet(nn.Module):
    """Attention U-Net for tumor segmentation with dynamic channel detection"""
    ...
    def __init__(self, n_channels=3, n_classes=1):
        ...
        # Dynamically get the actual channel sizes
        self.skip_channels = self._get_skip_channels()
        
        # Get bottleneck channels (output of last feature layer)
        self.bottleneck_channels = self._get_bottleneck_channels()
       
        # Decoder with dynamic channel sizes and channel adjustment
        self.up1 = nn.ConvTranspose2d(self.bottleneck_channels, 512, 2, stride=2)
        self.att1 = AttentionBlock(512, self.skip_channels[3], 256)
        # Add channel adjustment layer if needed
        self.skip_adjust1 = nn.Conv2d(self.skip_channels[3], 512, 1) if self.skip_channels[3] != 512 else nn.Identity()
        self.dec1 = self.conv_block(512 + 512, 512)
       
        self.up2 = nn.ConvTranspose2d(512, 256, 2, stride=2)
        self.att2 = AttentionBlock(256, self.skip_channels[2], 128)
        self.skip_adjust2 = nn.Conv2d(self.skip_channels[2], 256, 1) if self.skip_channels[2] != 256 else nn.Identity()
        self.dec2 = self.conv_block(256 + 256, 256)
       
        self.up3 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        self.att3 = AttentionBlock(128, self.skip_channels[1], 64)
        self.skip_adjust3 = nn.Conv2d(self.skip_channels[1], 128, 1) if self.skip_channels[1] != 128 else nn.Identity()
        self.dec3 = self.conv_block(128 + 128, 128)
       
        self.up4 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        self.att4 = AttentionBlock(64, self.skip_channels[0], 32)
        self.skip_adjust4 = nn.Conv2d(self.skip_channels[0], 64, 1) if self.skip_channels[0] != 64 else nn.Identity()
    ...
    def _get_skip_channels(self):
        """Dynamically determine the number of channels at each skip connection"""
    ...
    def _get_bottleneck_channels(self):
        """Get the number of channels from the bottleneck (last feature layer)"""
    ...
    def conv_block(self, in_channels, out_channels):
        return nn.Sequential(
            ...
            nn.GroupNorm(min(32, out_channels), out_channels),  # Use GroupNorm with proper group size
    ...
    def forward(self, x):
        ...
        # Decoder with attention and channel adjustment
        d1 = self.up1(x)
        # Resize features[0] (deepest) to match d1 spatial dimensions
        if features[0].shape[2:] != d1.shape[2:]:
            features[0] = F.interpolate(features[0], size=d1.shape[2:], mode='bilinear', align_corners=False)
        d1_att = self.att1(d1, features[0])
        features_adj1 = self.skip_adjust1(features[0])  # Adjust channels to match
        d1 = torch.cat([d1_att, features_adj1], dim=1)
        d1 = self.dec1(d1)
       
        d2 = self.up2(d1)
        if features[1].shape[2:] != d2.shape[2:]:
            features[1] = F.interpolate(features[1], size=d2.shape[2:], mode='bilinear', align_corners=False)
        d2_att = self.att2(d2, features[1])
        features_adj2 = self.skip_adjust2(features[1])
        d2 = torch.cat([d2_att, features_adj2], dim=1)
        d2 = self.dec2(d2)
       
        d3 = self.up3(d2)
        if features[2].shape[2:] != d3.shape[2:]:
            features[2] = F.interpolate(features[2], size=d3.shape[2:], mode='bilinear', align_corners=False)
        d3_att = self.att3(d3, features[2])
        features_adj3 = self.skip_adjust3(features[2])
        d3 = torch.cat([d3_att, features_adj3], dim=1)
        d3 = self.dec3(d3)
    
        d4 = self.up4(d3)
        if features[3].shape[2:] != d4.shape[2:]:
            features[3] = F.interpolate(features[3], size=d4.shape[2:], mode='bilinear', align_corners=False)
        d4_att = self.att4(d4, features[3])
        features_adj4 = self.skip_adjust4(features[3])
        d4 = torch.cat([d4_att, features_adj4], dim=1)
        d4 = self.dec4(d4)
