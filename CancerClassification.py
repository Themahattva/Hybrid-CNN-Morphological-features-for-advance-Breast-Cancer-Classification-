...
class BreakHisAnalyzer:
    ...
    def detect_dataset_structure(self):
        ...
        # Walk through all directories to understand structure
        for root, dirs, files in os.walk(self.dataset_path):
            ...
            # Check for cancer type patterns
    ...
    def load_dataset(self, selected_magnification='400X'):
        """Improved dataset loading with flexible structure detection"""
        class_counts = {}
        self.image_paths = []
        self.labels = []
        self.magnifications = []
        ...
        # First, detect the dataset structure
        ...
        # Strategy 1: Standard BreakHis structure
        success = self.load_standard_breakhis(selected_magnification, class_counts)
        
        if not success:
            # Strategy 2: Flexible structure based on folder names
            success = self.load_flexible_structure(selected_magnification, class_counts)
        
        if not success:
            # Strategy 3: Flat structure with filename-based classification
    ...
    def load_standard_breakhis(self, selected_magnification, class_counts):
        """Load standard BreakHis dataset structure"""
        try:
            # Look for standard BreakHis structure
            possible_structures = [
            ...
            for category_path, category_type in [(benign_path, 'benign'), (malignant_path, 'malignant')]:
                for cancer_type_folder in os.listdir(category_path):
                    ...
                    # Look for SOB_ folders or direct magnification folders
                    found_images = False
                    ...
                    # If no SOB_ folders, look for direct magnification folders
                    if not found_images:
    ...
    def load_flexible_structure(self, selected_magnification, class_counts):
        """Load dataset with flexible folder structure"""
        try:
            print("Trying flexible structure loading...")
            
            # Walk through all directories
            for root, dirs, files in os.walk(self.dataset_path):
                # Check if this directory contains the selected magnification
                if selected_magnification in os.path.basename(root):
                    images = self.get_images_from_folder(root)
                    if images:
                        # Try to determine cancer type from path
                ...
                # Check if any subdirectory is the magnification folder
                for dirname in dirs:
    ...
    def load_flat_structure(self, selected_magnification, class_counts):
        """Load dataset from flat structure using filename patterns"""
        try:
            ...
            for img_path in filtered_images:
                ...
                if cancer_type:
                    ...
                    # Default classification if type cannot be determined
    ...
    def infer_cancer_type_from_path(self, path):
        """Infer cancer type from file/folder path"""
        path_lower = path.lower()
        
        # Define mapping of keywords to cancer types
        type_keywords = {
            'adenosis': 'adenosis',
    ...
    def add_images_to_dataset(self, images, cancer_type, magnification, class_counts):
        """Add images to the dataset with proper labeling"""
