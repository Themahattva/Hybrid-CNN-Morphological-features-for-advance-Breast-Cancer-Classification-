...
class AttentionUNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        ...
        self.backbone_features = nn.ModuleList(list(backbone.features.children()))
        
        # Actual EfficientNet-B0 channels at layers [1,2,4,6]: [16, 24, 40, 80]
        # But layer 6 output is actually 80 channels, not 112
        
        self.up1 = nn.ConvTranspose2d(1280, 512, 2, stride=2)
        self.att1 = AttentionBlock(512, 80, 256)  # Changed from 112 to 80
        self.dec1 = self.conv_block(512 + 80, 512)
    ...
    def forward(self, x):
        ...
        # Decoder path
        d = self.up1(x)  # 512 channels
        
        # Attention with skip connection from layer 6 (80 channels)
        skip = F.interpolate(skip_connections[3], size=d.shape[2:], mode='bilinear', align_corners=False)
        att_skip = self.att1(d, skip)
        d = torch.cat([d, att_skip], dim=1)  # 512 + 80 = 592
        d = self.dec1(d)
        
        d = self.up2(d)  # 256 channels
        skip = F.interpolate(skip_connections[2], size=d.shape[2:], mode='bilinear', align_corners=False)
        att_skip = self.att2(d, skip)
        d = torch.cat([d, att_skip], dim=1)  # 256 + 40 = 296
        d = self.dec2(d)
        
        d = self.up3(d)  # 128 channels
        skip = F.interpolate(skip_connections[1], size=d.shape[2:], mode='bilinear', align_corners=False)
        att_skip = self.att3(d, skip)
        d = torch.cat([d, att_skip], dim=1)  # 128 + 24 = 152
        d = self.dec3(d)
        
        d = self.up4(d)  # 64 channels
        skip = F.interpolate(skip_connections[0], size=d.shape[2:], mode='bilinear', align_corners=False)
        att_skip = self.att4(d, skip)
        d = torch.cat([d, att_skip], dim=1)  # 64 + 16 = 80
        d = self.dec4(d)
        
        d = self.up5(d)  # 32 channels
        d = self.dec5(d)
        
        output = torch.sigmoid(self.final(d))
        return output
