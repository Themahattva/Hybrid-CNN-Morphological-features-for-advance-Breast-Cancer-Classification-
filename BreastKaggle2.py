...
class OptimizedCNNBackbone(nn.Module):
    def __init__(self):
        super(OptimizedCNNBackbone, self).__init__()
...
class HybridBreakHisClassifier(nn.Module):
    def __init__(self, num_classes=8, num_morphological_features=7, fusion_strategy='attention_weighted'):
        ...
        self.backbone = OptimizedCNNBackbone()
...
class BreakHisAnalyzer:
    ...
    def train_model(self, epochs=60, learning_rate=0.0005, fusion_strategy='attention_weighted', batch_size=64):
        ...
        for epoch in range(epochs):
            ...
            for batch_data in train_pbar:
                if len(batch_data) == 3:
                    ...
                    images = images.to(self.device, non_blocking=True)
                    labels = labels.to(self.device, non_blocking=True)
                    morph_features = torch.zeros((images.size(0), 7)).to(self.device, non_blocking=True)
    ...
    def save_model(self, filepath):
        ...
        torch.save({
            ...
            'class_mapping': self.class_mapping,
            'num_classes': len(self.label_encoder.classes_),
            'fusion_strategy': getattr(model_to_save, 'fusion_strategy', 'attention_weighted')
        }, filepath)
        return True
...
print("="*80)
print("HYBRID BREAKHIS BREAST CANCER CLASSIFICATION")
print("CNN + MORPHOLOGICAL FEATURES PIPELINE")
print("="*80)
...
training_configs = [
    {'fusion_strategy': 'attention_weighted', 'epochs': 60, 'lr': 0.0005, 'batch_size': 64},
    {'fusion_strategy': 'concatenate', 'epochs': 50, 'lr': 0.0005, 'batch_size': 64},
    {'fusion_strategy': 'separate_then_combine', 'epochs': 50, 'lr': 0.0005, 'batch_size': 64}
]
...
analyzer.load_model(f'hybrid_model_{best_strategy[0]}.pth')
