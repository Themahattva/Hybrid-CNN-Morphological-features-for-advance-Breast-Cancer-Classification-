...
class ImageProcessor:
    ...
    def extract_features(image, mask):
        ...
        if len(props) == 0:
            return np.zeros(7)  # Return zero features if no regions found
...
# NEW: Enhanced Dataset class that includes morphological features
...
# NEW: Hybrid CNN + Morphological Features Classifier
class HybridBreakHisClassifier(nn.Module):
    def __init__(self, num_classes=8, num_morphological_features=7, efficientnet_version='b0', fusion_strategy='concatenate'):
        ...
        # Final classifier based on fusion strategy
        if fusion_strategy == 'concatenate':
        ...
        elif fusion_strategy == 'attention_weighted':
            self.classifier = nn.Sequential(
                nn.Linear(512 + 128, 256),
                nn.ReLU(),
                nn.Dropout(0.4),
            ...
            # Separate classifiers for CNN and morphological features
    ...
    def forward(self, x, morphological_features):
        ...
        # Feature fusion based on strategy
        if self.fusion_strategy == 'concatenate':
            ...
            # Separate classification then weighted combination
...
# UPDATED: Enhanced Analyzer class with morphological feature integration
class BreakHisAnalyzer:
    def __init__(self, dataset_path):
        ...
        self.morph_scaler = StandardScaler()  # NEW: Scaler for morphological features
        self.image_paths = []
        self.labels = []
        self.magnifications = []
        self.morphological_features = []  # NEW: Store morphological features
        self.X_train = None
        self.X_test = None
        self.y_train = None
        self.y_test = None
        self.mag_train = None
        self.mag_test = None
        self.morph_train = None  # NEW: Morphological features for training
        self.morph_test = None   # NEW: Morphological features for testing
    ...
    # NEW: Extract morphological features for all images
    def extract_all_morphological_features(self, progress_callback=None):
        """Extract morphological features for all loaded images"""
    ...
    # Keep all the original dataset loading methods but add feature extraction
    def detect_dataset_structure(self):
        """Detect the structure of the dataset and return information about it"""
        structure_info = {
            'magnifications': set(),
            'cancer_types': set(),
            'total_images': 0,
    ...
    def load_dataset(self, selected_magnification='400X', extract_morphological=True):
        """Load dataset and optionally extract morphological features"""
        class_counts = {}
        self.image_paths = []
        self.labels = []
        self.magnifications = []
        ...
        # NEW: Extract morphological features if requested
    ...
    # Keep all original loading methods (load_standard_breakhis, load_flexible_structure, etc.)
    ...
    def split_dataset(self, test_size=0.2):
        """Split dataset including morphological features"""
        if len(self.morphological_features) > 0:
            self.X_train, self.X_test, self.y_train, self.y_test, self.mag_train, self.mag_test, self.morph_train, self.morph_test = train_test_split(
    ...
    def create_dataloaders(self, batch_size=16):
        """Create dataloaders with morphological features"""
    ...
    def train_model(self, epochs=50, learning_rate=0.001, fusion_strategy='attention_weighted', progress_callback=None):
        """Train hybrid model with CNN + morphological features"""
        num_classes = len(self.label_encoder.classes_)
        
        # NEW: Use hybrid model
    ...
    def process_and_predict_image(self, image_path):
        """Process and predict image with hybrid model"""
        if self.model is None:
            return None
        ...
        # Extract and normalize morphological features
        ...
        # Convert raw features back for display
        features_dict = {
    ...
    def save_model(self, filepath):
        """Save hybrid model with all components"""
        if self.model is None:
            return False
    ...
    def load_model(self, filepath, num_classes=None):
        """Load hybrid model with all components"""
...
class BreakHisGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Hybrid BreakHis: CNN + Morphological Features Classifier")
    ...
    def create_widgets(self):
        ...
        ttk.Label(title_frame, text="Hybrid BreakHis: CNN + Morphological Features Classifier", 
                 style='Title.TLabel').pack()
        ttk.Label(title_frame, text="Advanced Breast Cancer Classification with Multi-Modal Feature Fusion", 
    ...
    def create_control_panel(self, parent):
        ...
        self.load_btn = ttk.Button(dataset_frame, text="Load Dataset + Extract Features", 
        ...
        fusion_combo = ttk.Combobox(fusion_frame, textvariable=self.fusion_var, 
                                   values=["concatenate", "attention_weighted", "separate_then_combine"], 
                                   state="readonly", width=18)
        fusion_combo.pack(side=tk.LEFT, padx=(5, 0))
        
        self.train_btn = ttk.Button(training_frame, text="Train Hybrid Model", 
        ...
        ttk.Button(prediction_frame, text="Analyze with Hybrid Model", 
    ...
    def create_visualization_panel(self, parent):
        ...
        self.fig.suptitle('Hybrid BreakHis: CNN + Morphological Features Analysis', fontsize=14, fontweight='bold')
        ...
        self.axes[1, 0].text(0.5, 0.5, 'No Features\nExtracted', ha='center', va='center', 
                            fontsize=12, transform=self.axes[1, 0].transAxes, color='gray')
    ...
    def load_dataset(self):
        ...
        try:
            ...
            info_text += f"✓ Morphological features extracted\n"
            info_text += f"✓ CNN + Morph fusion ready"
    ...
    def analyze_image(self):
        ...
        image_path = filedialog.askopenfilename(
            title="Select Cancer Image for Hybrid Analysis",
    ...
    def display_analysis_results(self, result):
        """Display results from hybrid analysis"""
        for i in range(2):
            for j in range(3):
                self.axes[i, j].clear()
        ...
        # Update figure title with hybrid prediction
        fusion_strategy = getattr(self.analyzer.model, 'fusion_strategy', 'Unknown')
        self.fig.suptitle(f'Hybrid Classification ({fusion_strategy}): {pred["class"]} (Confidence: {pred["confidence"]:.3f})', 
                         fontsize=14, fontweight='bold')
        
        self.canvas.draw()
        
    def plot_morphological_features(self, morph_features):
        """Plot morphological features as bar chart"""
        self.axes[1, 0].clear()
        ...
        for bar, raw_val, norm_val in zip(bars, morph_features, normalized_features):
            ...
            self.axes[1, 0].text(bar.get_x() + bar.get_width()/2., height + 0.02,
                               f'{raw_val:.2f}', ha='center', va='bottom', fontsize=7)
    ...
    def update_prediction_text(self, result, filename):
        """Update prediction text with hybrid analysis results"""
        pred = result['prediction']
        ...
        analysis_text += f"4. ✓ 7 Morphological features extracted\n"
        analysis_text += f"5. ✓ Features normalized with StandardScaler\n"
        analysis_text += f"6. ✓ CNN features extracted (EfficientNet)\n"
        analysis_text += f"7. ✓ Multi-modal feature fusion applied\n"
        analysis_text += f"8. ✓ Final hybrid classification performed\n"
        ...
        if morph_features[2] > 0.8:  # Eccentricity
            analysis_text += f"• High eccentricity ({morph_features[2]:.3f}) - Elongated shape\n"
        elif morph_features[2] < 0.3:
            analysis_text += f"• Low eccentricity ({morph_features[2]:.3f}) - Circular shape\n"
        
        if morph_features[3] > 0.9:  # Solidity
            analysis_text += f"• High solidity ({morph_features[3]:.3f}) - Smooth boundary\n"
        elif morph_features[3] < 0.7:
            analysis_text += f"• Low solidity ({morph_features[3]:.3f}) - Irregular boundary\n"
        
        if morph_features[6] > 2.0:  # Compactness
            analysis_text += f"• High compactness ({morph_features[6]:.3f}) - Complex perimeter\n"
        elif morph_features[6] < 1.2:
            analysis_text += f"• Low compactness ({morph_features[6]:.3f}) - Simple perimeter\n"
        
        if tumor_type == "MALIGNANT":
            analysis_text += f"\n⚠️  MALIGNANT TISSUE DETECTED (HYBRID)\n"
            analysis_text += f"   Enhanced accuracy with morphological analysis\n"
            analysis_text += f"   Requires immediate clinical attention\n"
        else:
            analysis_text += f"\n✓  BENIGN TISSUE DETECTED (HYBRID)\n"
            analysis_text += f"   Multi-modal confirmation of benign status\n"
            analysis_text += f"   Continue routine monitoring\n"
        
        analysis_text += f"\nMODEL ADVANTAGES:\n"
        analysis_text += f"• Combines CNN visual features with quantitative morphology\n"
        analysis_text += f"• Attention-weighted fusion for optimal feature combination\n"
        analysis_text += f"• Enhanced interpretability through feature analysis\n"
        analysis_text += f"• Robust classification with multi-modal evidence\n"
        ...
        if file_path:
            try:
                ...
                if success:
                    ...
                    fusion_strategy = getattr(self.analyzer.model, 'fusion_strategy', 'Unknown')
                    self.model_status.configure(text=f"Model loaded: {os.path.basename(file_path)} ({fusion_strategy})", 
